...
class RoleRoutingService {
  ...
  static Future<bool> navigateByRole(BuildContext context, AuthFlowNotifier authFlow) async {
    try {
      // Check mobile app access first
      await checkMobileAppAccess();
    } catch (e) {
      // Access denied - logout and show message
    ...
    // Access granted - fetch role and navigate
    final userId = _supabase.auth.currentUser?.id;
    if (userId == null) {
      await _supabase.auth.signOut();
      authFlow.forceLogout();
    ...
    // Navigate based on role (already validated by checkMobileAppAccess)
    Widget targetScreen;
    switch (role) {
      case 'customer':
        ...
        break;
      case 'staff':
        // Staff with staff_type='collection' can access
        targetScreen = StaffDashboard(staffId: profileId);
        break;
      default:
        // Should not reach here (admin blocked by checkMobileAppAccess)
        await _supabase.auth.signOut();
        authFlow.forceLogout();
        if (context.mounted) {
          Navigator.of(context).pushAndRemoveUntil(
